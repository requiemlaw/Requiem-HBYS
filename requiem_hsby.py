...
class Database:
    ...
    def _seed_ilk_veri(self):
        ...
        if self.sorgula("SELECT COUNT(*) as c FROM doktorlar")[0]["c"] == 0:
            self.calistir("""INSERT INTO doktorlar (ad_soyad, uzmanlik, poliklinik, uygunluk)
                              VALUES (?,?,?,?)""", ("TEMEL BULUT", "Genel Cerrahi", "Genel Cerrahi", "Müsait"))
            self.calistir("""INSERT INTO doktorlar (ad_soyad, uzmanlik, poliklinik, uygunluk)
                              VALUES (?,?,?,?)""", ("AYŞE YILMAZ", "Kardiyoloji", "Kardiyoloji", "Müsait"))
        ...
        if self.sorgula("SELECT COUNT(*) as c FROM personeller")[0]["c"] == 0:
            self.calistir("""INSERT INTO personeller (ad_soyad, rol, birim, telefon, kullanici_adi, sifre, durum)
                              VALUES (?,?,?,?,?,?,?)""",
                          ("Sistem Yöneticisi", "Yönetici", "Genel Yönetim", "-", "admin", "1234", "Aktif"))
            self.calistir("""INSERT INTO personeller (ad_soyad, rol, birim, telefon, kullanici_adi, sifre, durum)
                              VALUES (?,?,?,?,?,?,?)""",
                          ("TEMEL BULUT", "Doktor", "Genel Cerrahi", "-", "temel.bulut", "1234", "Aktif"))
    ...
    def giris_kontrol(self, kadi, sifre):
        r = self.sorgula("SELECT * FROM personeller WHERE kullanici_adi=? AND sifre=? AND durum='Aktif'", (kadi, sifre))
    ...
    def hasta_getir_tek(self, hid):
        r = self.sorgula("SELECT * FROM hastalar WHERE id=?", (hid,))
        return r[0] if r else None
    ...
    def personel_ekle(self, v):
        return self.calistir("""INSERT INTO personeller (ad_soyad, rol, birim, telefon, kullanici_adi, sifre, durum)
                                 VALUES (?,?,?,?,?,?,?)""",
                             (v["ad_soyad"], v["rol"], v["birim"], v["telefon"], v["kullanici_adi"], v["sifre"], v["durum"]))
    ...
    def randevu_getir_tek(self, rid):
        r = self.sorgula("""SELECT r.*, h.ad_soyad as hasta_ad, d.ad_soyad as doktor_ad FROM randevular r
    ...
    def istatistikleri_getir(self):
        ...
        return toplam_hasta, bugunku_randevu, yatan_hasta
...
class RequiemHBYSPro:
    ...
    def dashboard_guncelle(self):
        toplam_hasta, bugunku_randevu, yatan_hasta = self.db.istatistikleri_getir()
    ...
    def sistem_yoneticisi_modulunu_ac(self):
        ...
        kolonlar = ("id", "ad", "rol", "birim", "tel", "kadi", "durum")
        basliklar = {"id": "ID", "ad": "Adı Soyadı", "rol": "Rol", "birim": "Birim", "tel": "Telefon",
                    "kadi": "Kullanıcı Adı", "durum": "Durum"}
        genislikler = {"id": 40, "ad": 180, "rol": 100, "birim": 150, "tel": 110, "kadi": 120, "durum": 70}
        tree = self._treeview_olustur(liste_frame, kolonlar, basliklar, genislikler)

        def listeyi_doldur():
            tree.delete(*tree.get_children())
            for p in self.db.personelleri_getir():
                tree.insert("", "end", iid=str(p["id"]), values=(p["id"], p["ad_soyad"], p["rol"], p["birim"],
                                                                   p["telefon"], p["kullanici_adi"], p["durum"]))

        def secileni_yukle(event=None):
            sec = tree.selection()
            if not sec:
                return
            p = next((x for x in self.db.personelleri_getir() if x["id"] == int(sec[0])), None)
            if not p:
                return
            e_ad.delete(0, "end"); e_ad.insert(0, p["ad_soyad"])
            cb_rol.set(p["rol"]); cb_birim.set(p["birim"])
            e_tel.delete(0, "end"); e_tel.insert(0, p["telefon"])
            e_kadi.delete(0, "end"); e_kadi.insert(0, p["kullanici_adi"])
            e_sifre.delete(0, "end"); e_sifre.insert(0, p["sifre"])
            cb_durum.set(p["durum"])

        tree.bind("<<TreeviewSelect>>", secileni_yukle)

        def personel_kaydet():
            if not e_ad.get().strip() or not cb_rol.get() or not e_kadi.get().strip():
                messagebox.showwarning("Uyarı", "Adı Soyadı, Rol ve Kullanıcı Adı zorunludur!", parent=pencere)
                return
            v = {
                "ad_soyad": e_ad.get().strip(), "rol": cb_rol.get(), "birim": cb_birim.get(),
                "telefon": e_tel.get().strip(), "kullanici_adi": e_kadi.get().strip(),
                "sifre": e_sifre.get().strip() or "1234", "durum": cb_durum.get()
            }
            sec = tree.selection()
            try:
                if sec:
                    self.db.personel_guncelle(int(sec[0]), v)
                    messagebox.showinfo("Bilgi", "Personel bilgileri güncellendi.", parent=pencere)
                else:
                    self.db.personel_ekle(v)
                    messagebox.showinfo("Bilgi", "Yeni personel başarıyla kaydedildi.", parent=pencere)
            except sqlite3.IntegrityError:
                messagebox.showerror("Hata", "Bu kullanıcı adı zaten kullanılıyor!", parent=pencere)
                return
            listeyi_doldur()
            for e in (e_ad, e_tel, e_kadi, e_sifre):
                e.delete(0, "end")
            cb_rol.set(""); cb_birim.set(""); cb_durum.set("Aktif")

        def personel_sil():
            sec = tree.selection()
            if not sec:
                messagebox.showwarning("Uyarı", "Silinecek personeli seçin.", parent=pencere)
                return
            self.db.personel_sil(int(sec[0]))
            listeyi_doldur()

        btn_frame = tk.Frame(form, bg="#f0f0f0")
        btn_frame.grid(row=4, column=0, columnspan=4, pady=8)
        tk.Button(btn_frame, text="Kaydet / Güncelle", bg="#4CAF50", fg="white", width=18, command=personel_kaydet).pack(side="left", padx=5)
        tk.Button(btn_frame, text="Sil", bg="#f44336", fg="white", width=12, command=personel_sil).pack(side="left", padx=5)

        listeyi_doldur()

    # =================================================================
    #  6) TETKİK İSTEM MODÜLÜ (hasta seçilebilir + DB'ye kayıt + geçmiş)
    # =================================================================
    def tetkik_istem_modulunu_ac(self):
        if not self.db.hastalari_getir():
            messagebox.showwarning("Uyarı", "Tetkik istemi oluşturmadan önce Hasta Kayıt modülünden hasta ekleyin.", parent=self.root)
            return

        self.pencere = tk.Toplevel(self.root)
        self.pencere.title("Tetkik İstem - Laboratuvar ve Görüntüleme")
        self.pencere.geometry("1300x800")
        self.pencere.configure(bg="#f0f0f0")

        self.checkbox_vars = {}

        self.ust_hasta_paneli_kur()
        self.alt_secilenler_paneli_kur()
        self.orta_sekmeli_yapi_kur()

    def ust_hasta_paneli_kur(self):
        frame = tk.LabelFrame(self.pencere, text="Hasta ve Müracaat Bilgileri", bg="#e4e4e4", font=("Arial", 9, "bold"))
        frame.pack(fill="x", padx=10, pady=5)

        tk.Label(frame, text="Hasta:", bg="#e4e4e4", font=("Arial", 9, "bold")).grid(row=0, column=0, sticky="e", padx=5, pady=4)
        self.tetkik_cb_hasta = ttk.Combobox(frame, width=30, state="readonly")
        self.tetkik_cb_hasta.grid(row=0, column=1, sticky="w", padx=5, pady=4)
        self.tetkik_cb_hasta["values"] = [f'{h["id"]} - {h["ad_soyad"]}' for h in self.db.hastalari_getir()]
        self.tetkik_cb_hasta.current(0)

        tk.Label(frame, text="İsteyen Doktor:", bg="#e4e4e4", font=("Arial", 9, "bold")).grid(row=0, column=2, sticky="e", padx=5, pady=4)
        self.tetkik_cb_doktor = ttk.Combobox(frame, width=30, state="readonly")
        self.tetkik_cb_doktor.grid(row=0, column=3, sticky="w", padx=5, pady=4)
        self.tetkik_cb_doktor["values"] = [f'{d["id"]} - {d["ad_soyad"]} ({d["poliklinik"]})' for d in self.db.doktorlari_getir()]
        if self.db.doktorlari_getir():
            self.tetkik_cb_doktor.current(0)

        tk.Label(frame, text="Müracaat Tarihi:", bg="#e4e4e4", font=("Arial", 9, "bold")).grid(row=1, column=0, sticky="e", padx=5, pady=4)
        e_tarih = tk.Entry(frame, width=30)
        e_tarih.grid(row=1, column=1, sticky="w", padx=5, pady=4)
        e_tarih.insert(0, datetime.now().strftime("%d.%m.%Y"))
        e_tarih.config(state='readonly')

        tk.Label(frame, text="Müracaat No:", bg="#e4e4e4", font=("Arial", 9, "bold")).grid(row=1, column=2, sticky="e", padx=5, pady=4)
        e_no = tk.Entry(frame, width=30)
        e_no.grid(row=1, column=3, sticky="w", padx=5, pady=4)
        e_no.insert(0, str(random.randint(100000, 999999)))
        e_no.config(state='readonly')

    def orta_sekmeli_yapi_kur(self):
        self.notebook = ttk.Notebook(self.pencere)
        self.notebook.pack(fill="both", expand=True, padx=10, pady=5)

        tab_lab = ttk.Frame(self.notebook)
        tab_goruntu = ttk.Frame(self.notebook)

        self.notebook.add(tab_lab, text="Laboratuvar")
        self.notebook.add(tab_goruntu, text="Görüntüleme")

        self.goruntuleme_icerigi_kur(tab_goruntu)
        self.laboratuvar_icerigi_kur(tab_lab)

        self.notebook.select(tab_goruntu)

    def goruntuleme_icerigi_kur(self, parent_tab):
        sol_menu = tk.Frame(parent_tab, width=200, bg="#dcdcdc", relief="sunken", bd=2)
        sol_menu.pack(side="left", fill="y")

        sag_icerik = tk.Frame(parent_tab, bg="white")
        sag_icerik.pack(side="right", fill="both", expand=True)

        kategoriler = ["Röntgen", "Manyetik Rezonans (MR)", "Bilgisayarlı Tomografi (BT)", "Patolojik Tetkikler",
                       "Mamografi", "Ultrason - Doppler"]
        for kat in kategoriler:
            tk.Button(sol_menu, text=kat, bg="#e8e8e8", anchor="w", width=25).pack(fill="x", pady=1)

        tk.Label(sag_icerik, text="Manyetik Rezonans (MR) Tetkikleri", font=("Arial", 10, "bold"), bg="white").pack(
            anchor="w", padx=10, pady=5)

        checkbox_frame = tk.Frame(sag_icerik, bg="white")
        checkbox_frame.pack(fill="both", expand=True, padx=10)

        mr_listesi = [
            "MRG, Abdomen - alt, kontrastsız", "MRG, Abdomen - üst, kontrastlı", "MRG, Pelvis, kontrastsız",
            "MRG, Pelvis, kontrastlı",
            "MRG, Prostat, dinamik kontrastlı", "MRG, Ayak - sağ, kontrastlı", "MRG, Uyluk - sol, kontrastlı",
            "MRG, Meme - bilateral",
            "MRG anjiyografi, Beyin", "MRG, Beyin, spektroskopi", "MRG, Beyin, kontrastlı", "MRG, Servikal vertebra",
            "MRG, Lomber vertebra, kontrastlı", "MRG, Torakal vertebra", "MRG, Diz - sağ, kontrastlı",
            "MRG, Diz - sol, kontrastsız",
            "MRG, Omuz - sağ, kontrastlı", "MRG, El bileği - sol", "MRG, Kranyofasiyal", "MRG, Orbita - bilateral"
        ]

        for idx, tetkik in enumerate(mr_listesi):
            kodu = f"MRG{100 + idx}"
            var = tk.IntVar()
            self.checkbox_vars[kodu] = var
            chk = tk.Checkbutton(checkbox_frame, text=tetkik, variable=var, bg="white", font=("Arial", 8),
                                 command=lambda t=tetkik, v=var, k=kodu: self.tetkik_listesini_guncelle(t, v, k))
            chk.grid(row=idx // 4, column=idx % 4, sticky="w", padx=10, pady=2)

    def laboratuvar_icerigi_kur(self, parent_tab):
        sag_icerik = tk.Frame(parent_tab, bg="white")
        sag_icerik.pack(side="right", fill="both", expand=True)

        tk.Label(sag_icerik, text="Sık İstenen Laboratuvar Testleri", font=("Arial", 10, "bold"), bg="white").pack(
            anchor="w", padx=10, pady=5)
        ...
        lab_listesi = [
            ...
            "Kalsiyum (Ca)", "Demir (Fe)", "Demir Bağlama Kapasitesi", "Ferritin", "Vitamin B12",
            "Hemogram (Tam Kan Sayımı)", "TSH", "Serbest T4", "CRP"
        ]

        for idx, tetkik in enumerate(lab_listesi):
            kodu = f"LAB{200 + idx}"
            var = tk.IntVar()
            self.checkbox_vars[kodu] = var
            chk = tk.Checkbutton(checkbox_frame, text=tetkik, variable=var, bg="white", font=("Arial", 8),
    ...
    def tetkik_listesini_guncelle(self, tetkik_adi, var, kodu):
        if var.get() == 1:
            doktor_ad = "-"
            if self.tetkik_cb_doktor.get():
                doktor_ad = self.tetkik_cb_doktor.get().split(" - ", 1)[1].split(" (")[0]
            self.tree.insert("", "end", iid=kodu, values=(kodu, tetkik_adi, "1", "İstem Bekliyor", doktor_ad))
        else:
            if self.tree.exists(kodu):
                self.tree.delete(kodu)

    def alt_secilenler_paneli_kur(self):
        frame = tk.LabelFrame(self.pencere, text="İstenen Tetkikler (Barkod Listesi)", font=("Arial", 9, "bold"))
        frame.pack(fill="x", side="bottom", padx=10, pady=10)

        kolonlar = ("kodu", "tetkik_adi", "adet", "durum", "isteyen_doktor")
        self.tree = ttk.Treeview(frame, columns=kolonlar, show="headings", height=8)

        basliklar = {"kodu": "Kodu", "tetkik_adi": "Tetkik Adı", "adet": "Adet", "durum": "Lab Durumu",
                    "isteyen_doktor": "İsteyen Doktor"}
        genislik = {"kodu": 100, "tetkik_adi": 400, "adet": 50, "durum": 150, "isteyen_doktor": 200}

        for col in kolonlar:
            self.tree.heading(col, text=basliklar[col])
            self.tree.column(col, width=genislik[col])

        self.tree.pack(fill="x", padx=5, pady=5)

        btn_frame = tk.Frame(frame)
        btn_frame.pack(anchor="e", padx=5, pady=5)
        tk.Button(btn_frame, text="Geçmiş İstekler", bg="#2196F3", fg="white", font=("Arial", 9, "bold"),
                  command=self.gecmis_istekleri_goster, width=15).pack(side="left", padx=5)
        tk.Button(btn_frame, text="Kaydet (F2)", bg="#4CAF50", fg="white", font=("Arial", 9, "bold"),
                  command=self.tetkikleri_kaydet, width=15).pack(side="left", padx=5)
        tk.Button(btn_frame, text="İptal / Çıkış", bg="#f44336", fg="white", font=("Arial", 9, "bold"),
                  command=self.pencere.destroy, width=15).pack(side="left")

    def tetkikleri_kaydet(self):
        if not self.tree.get_children():
            messagebox.showwarning("Uyarı", "Lütfen kaydedilecek en az bir tetkik seçin!", parent=self.pencere)
            return
        if not self.tetkik_cb_hasta.get() or not self.tetkik_cb_doktor.get():
            messagebox.showwarning("Uyarı", "Lütfen hasta ve isteyen doktor seçin!", parent=self.pencere)
            return

        hasta_id = int(self.tetkik_cb_hasta.get().split(" - ")[0])
        doktor_ad = self.tetkik_cb_doktor.get().split(" - ", 1)[1].split(" (")[0]

        for item in self.tree.get_children():
            kodu, tetkik_adi, adet, durum, _ = self.tree.item(item, "values")
            self.db.tetkik_ekle(hasta_id, kodu, tetkik_adi, int(adet), "İstem Bekliyor", doktor_ad)

        toplam_tetkik = len(self.tree.get_children())
        messagebox.showinfo("Başarılı",
                            f"{toplam_tetkik} adet tetkik sisteme başarıyla kaydedildi ve barkodları oluşturuldu.",
                            parent=self.pencere)
    ...
    def gecmis_istekleri_goster(self):
        pencere = tk.Toplevel(self.pencere)
        pencere.title("Geçmiş Tetkik İstekleri")
        pencere.geometry("950x500")
        pencere.configure(bg="#f0f0f0")

        kolonlar = ("id", "hasta", "kodu", "ad", "adet", "durum", "doktor", "tarih")
        basliklar = {"id": "ID", "hasta": "Hasta", "kodu": "Kodu", "ad": "Tetkik Adı", "adet": "Adet",
                    "durum": "Durum", "doktor": "İsteyen Doktor", "tarih": "Tarih"}
        genislikler = {"id": 40, "hasta": 160, "kodu": 80, "ad": 260, "adet": 50, "durum": 110, "doktor": 150, "tarih": 90}
        tree = self._treeview_olustur(pencere, kolonlar, basliklar, genislikler, height=20)

        for t in self.db.tetkikleri_getir():
            tree.insert("", "end", values=(t["id"], t["hasta_ad"], t["kodu"], t["tetkik_adi"], t["adet"],
                                            t["durum"], t["isteyen_doktor"], t["tarih"]))
